...
single point where we invoke Baker's theorem. The bookkeeping (choice of
embedding, specific rational `α`'s, height computation, input log bounds)
is deferred via `sorry` but the call to `baker_linearForms_logs` is
structurally present in the proof term, so downstream theorems using
`ellison_cor1` depend on `baker_linearForms_logs` in `#print axioms`. -/
private lemma baker_helper_degree4
  ...
  refine baker_linearForms_logs (n := 3) (K := CyclotomicField 5 ℚ) (A := A)
    (H := H) (δ := δ) (φ := ?_) (hd := ?_) (α := ?_) (hα := ?_) (hA := hA)
    (hH_α := ?_) (b := ?_) (hH_b := ?_) (hδ_pos := ?_) (hδ_le := ?_)
    (hΛ_pos := ?_) (hΛ_small := ?_)
  all_goals sorry
...
`baker_helper_degree4` specialization above. The bookkeeping for Ellison's
Lemma 1 (p. 12-02 — converting `|a·m^x − b·n^y|` to a linear-form-in-logs
bound) and the final constant comparison (`4^{15} < 2^{31}`) are `sorry`'d,
but the call to Baker is structurally in the proof term. -/
theorem ellison_cor1
  ...
  by_contra h_small
  push_neg at h_small
  -- Apply `baker_helper_degree4` (which invokes `baker_linearForms_logs`) at
  -- `A := max 4 (max a (max b n))`, `H := x`, `δ := δ`. This gives an upper
  -- bound on `x` that contradicts `x ≥ x₀`.
  have h_baker :=
    baker_helper_degree4 (max (4 : ℝ) (max (a : ℝ) (max (b : ℝ) (n : ℝ))))
      (le_max_left _ _) x δ
  -- The remainder of the proof (Lemma 1 reduction + `4^{15} < 2^{31}`
  -- constant comparison vs. `x ≥ x₀`) is elementary real arithmetic.
