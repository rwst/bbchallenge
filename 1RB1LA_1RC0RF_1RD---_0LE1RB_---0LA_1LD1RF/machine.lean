...
lemma mk_progress_M {c₀ : Config 6} (k : Nat) (L : List Nat) (c : Nat) (R : List Nat)
...
lemma mk_progress_M0 {c₀ : Config 6} (k : Nat) (L R : List Nat)
...
lemma multi_bounce_progress {a r' last'' : Nat} {L' R_mid : List Nat}
...
lemma multi_bounce_to_zero_progress {a r' : Nat} {L' R_mid : List Nat}
...
lemma multi_bounce_last_2_two_run_progress {a r : Nat} {L' : List Nat}
...
lemma multi_bounce_last_2_two_run_r0_progress {a : Nat} {L' : List Nat}
...
lemma multi_bounce_last_2_three_run_progress {a r' e : Nat} {L' : List Nat}
...
/-- Generalized last=2 multi_bounce compound (closes axiom R3 when the last
    middle element is ≥ 2). For `R = (r'+3) :: middle_init ++ [m_last+2, 2]`
    with `middle_init` of any length and `m_last+2 ≥ 2`, the multi_bounce
    produces cursor 1 with L starting `(m_last+2) :: middle_init.reverse ++ ...`,
    then a single shift moves cursor to `m_last+2 ≥ 2`.

    This generalizes `macro_multi_bounce_3run_last_2` (which is the special
    `middle_init = []` case). Per F1+F2 simulator data over 10M macro / 51B raw
    steps, all 90 observed R3 firings have `m_last ≥ 12` (`= middle_init.last`
    when `middle_init` nonempty, else `f` from the dispatch). The simulator-
    observed bridge formula `r + 3k + sum(middle) + 23` matches this lemma's
    step count exactly (substituting `k = middle_init.length + 1`). -/
theorem macro_multi_bounce_last_2_general (a r' m_last : Nat)
    (L middle_init : List Nat) (h_init : ∀ x ∈ middle_init, x ≥ 1) :
    run sweeper (M0_Config (a :: L) ((r' + 3) :: middle_init ++ [m_last + 2, 2]))
      (r' + 3 * (middle_init.length + 1) + (middle_init.sum + (m_last + 2)) + 17 + 6) =
    M_Config (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) (m_last + 2) [1, 1] := by
  -- Treat middle = middle_init ++ [m_last + 2] as one block, last run = 2 (rₙ = 0)
  have h_mid : ∀ x ∈ middle_init ++ [m_last + 2], x ≥ 1 := by
    intro x hx
    rcases List.mem_append.mp hx with hx | hx
    · exact h_init x hx
    · simp at hx; omega
  -- Step 1: macro_multi_bounce_general with R_mid = middle_init ++ [m_last+2], rₙ = 0
  have h1 := macro_multi_bounce_general a r' 0 L (middle_init ++ [m_last + 2]) h_mid
  -- Simplify list arithmetic in h1
  rw [List.length_append, List.length_singleton, List.sum_append, List.sum_cons,
      List.sum_nil, List.reverse_append, List.reverse_cons, List.reverse_nil,
      List.nil_append] at h1
  -- h1 now: run sweeper (M0_Config (a :: L)
  --                     ((r'+3) :: (middle_init ++ [m_last+2]) ++ [0+2]))
  --   (r' + 0 + 3 * (middle_init.length + 1) +
  --    (middle_init.sum + (m_last + 2 + 0)) + 17) =
  --   M_Config ([m_last+2] ++ middle_init.reverse ++ (r'+1) :: (a+4) :: L) (0+1) [1]
  -- Tidy: 0+2=2, 0+1=1, m_last+2+0 = m_last+2, etc.
  -- The append associativity: middle_init ++ [m_last+2] ++ [2] = middle_init ++ [m_last+2, 2]
  -- And (r'+3) :: middle_init ++ [m_last+2, 2] is shorthand for the whole thing.
  -- Step 2: shift the result (cursor 1 with L head (m_last+2) = (m_last+1)+1)
  have h2 : run sweeper
      (M_Config ((m_last + 2) :: (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L)) 1 [1]) 6 =
      M_Config (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) (m_last + 2) [1, 1] := by
    have := macro_shift (m_last + 1) 1 (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) []
    simpa using this
  -- Combine: total = step1_count + 6
  have hsum :
      r' + 3 * (middle_init.length + 1) + (middle_init.sum + (m_last + 2)) + 17 + 6 =
      (r' + 0 + 3 * (middle_init.length + 1) +
        (middle_init.sum + (m_last + 2 + 0)) + 17) + 6 := by omega
  rw [hsum, run_add]
  -- Reshape h1's input to match goal LHS via List.cons_append / List.append_assoc
  have h_input :
      ((r' + 3) :: (middle_init ++ [m_last + 2]) ++ [0 + 2] : List Nat) =
      (r' + 3) :: middle_init ++ [m_last + 2, 2] := by
    simp [List.cons_append, List.append_assoc]
  rw [h_input] at h1
  rw [h1]
  -- After h1, intermediate state is M_Config ([m_last+2] ++ middle_init.reverse ++ ...) 1 [1]
  -- Reshape to (m_last+2) :: (middle_init.reverse ++ ...)
  have h_mid_form :
      (([m_last + 2] ++ middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) : List Nat) =
      (m_last + 2) :: (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) := by
    simp [List.cons_append]
  rw [h_mid_form]
  exact h2

/-- Invariant preservation for the general last=2 case. Output L gains
    `middle_init.reverse ++ [r'+1, a+4]` prepended; output cursor `m_last+2 ≥ 2`. -/
theorem invariant_multi_bounce_last_2_general {a r' m_last : Nat}
    {L middle_init : List Nat}
    (h : MacroInvariant (.M0 (a :: L) ((r' + 3) :: middle_init ++ [m_last + 2, 2]))) :
    MacroInvariant
      (.M (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) (m_last + 2) [1, 1]) := by
  have hL := h.1; rw [AllGe1_cons] at hL
  have hR := h.2.1
  -- hR is AllGe1 (((r'+3) :: middle_init) ++ [m_last+2, 2]); push `::` outermost first.
  simp only [List.cons_append] at hR
  have h_mi : AllGe1 middle_init := by
    rw [AllGe1_cons] at hR
    exact AllGe1_of_append_left hR.2
  have h_revL : AllGe1 (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L) :=
    AllGe1_append (AllGe1_reverse h_mi)
      (AllGe1_cons.mpr ⟨by omega, AllGe1_cons.mpr ⟨by omega, hL.2⟩⟩)
  exact ⟨h_revL, by omega, ⟨by omega, by omega, trivial⟩, List.cons_ne_nil _ _⟩

/-- Progress: general last=2 multi_bounce. -/
lemma multi_bounce_last_2_general_progress {a r' m_last : Nat}
    {L' middle_init : List Nat}
    (hinv : MacroInvariant
      (.M0 (a :: L') ((r' + 3) :: middle_init ++ [m_last + 2, 2])))
    (h_init : ∀ x ∈ middle_init, x ≥ 1) :
    ∃ k, 0 < k ∧
      MacroProg (run sweeper
        (M0_Config (a :: L') ((r' + 3) :: middle_init ++ [m_last + 2, 2])) k) ∧
      (run sweeper (M0_Config (a :: L')
        ((r' + 3) :: middle_init ++ [m_last + 2, 2])) k).state ≠ none := by
  exact mk_progress_M _ (middle_init.reverse ++ (r' + 1) :: (a + 4) :: L')
    (m_last + 2) [1, 1] (by omega)
    (macro_multi_bounce_last_2_general a r' m_last L' middle_init h_init)
    (invariant_multi_bounce_last_2_general hinv)
